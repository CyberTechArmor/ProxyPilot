...
test('FP-3 permanent master/application users resume without replacement; retirement requires fresh administration, linked SSO and separate recovery',()=>withDb(async db=>{
  ...
  // Both permanent identities are created email-verified (SSO sign-in and linking need it); the application identity must register a passkey at first sign-in.
  for(const realm of ['master','proxypilot'])assert.equal(wire.realms.get(realm).users.find(u=>u.username==='alice').emailVerified,true);
  assert.deepEqual(wire.realms.get('proxypilot').users.find(u=>u.username==='alice').requiredActions,['webauthn-register-passwordless']);assert.deepEqual(wire.realms.get('master').users.find(u=>u.username==='alice').requiredActions,[]);
  // An owned identity created unverified by an earlier version is corrected on the next run.
  wire.realms.get('proxypilot').users.find(u=>u.username==='alice').emailVerified=false;await create();assert.equal(wire.realms.get('proxypilot').users.find(u=>u.username==='alice').emailVerified,true);
