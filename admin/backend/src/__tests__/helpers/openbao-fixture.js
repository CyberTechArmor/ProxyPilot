...
export function baoFixture(db,{initialized=false,sealed=true}={}){const resources=new Map(),policies=new Map(),mounts={},auth={},calls=[],tokens=new Set(),secretIds=new Map();let unavailable=false,progress=0,initCount=0,cluster='g6-cluster',broad=false,revocations=0;
  const send=async(_origin,path,{method='GET',token,body}={})=>{calls.push({path,method});if(unavailable)throw Error('SENSITIVE-UPSTREAM-DO-NOT-LOG');const yes=b=>({status:200,body:b}),no=(code=403)=>({status:code,body:null}),r=readOpenBao(db),n=namesFor(r);
    ...
    if(path==='/v1/sys/init'){if(initialized)return no(400);initialized=true;sealed=true;initCount++;return yes({keys_base64:[encrypted,encrypted,encrypted],root_token:encrypted});}
    if(path==='/v1/sys/unseal'){if(body.key!=='validDisposableShare12345')return no(400);if(++progress>=2)sealed=false;return yes({sealed,progress});}
    ...
    const root=token==='g6-bootstrap-token'&&revocations===0,machine=tokens.has(token);if(!root&&!machine)return no();
    if(path==='/v1/auth/token/revoke-self'){if(root)revocations++;else tokens.delete(token);return {status:204,body:null};}
    if(path==='/v1/auth/token/lookup-self')return yes({data:{policies:root?['root']:[n.machine]}});
    if(path==='/v1/sys/capabilities-self')return yes({data:Object.fromEntries(body.paths.map(p=>[p,p===`${n.database}/creds/reader`?['read']:['deny']]))});
  ...
  return {send,calls,resources,policies,mounts,auth,tokens,secretIds,get initCount(){return initCount;},get revocations(){return revocations;},get sealed(){return sealed;},set sealed(v){sealed=v;progress=0;},set initialized(v){initialized=v;},set unavailable(v){unavailable=v;},set cluster(v){cluster=v;},set broad(v){broad=v;},resetBootstrap(){revocations=0;}};
}
