...
// Production command, executor, credential store and Vaultwarden access adapters;
// only external probes/transport and the database consumer are scripted.
import { DatabaseSync } from 'node:sqlite';
import { registerHooks } from 'node:module';
let sequence = 0;
const pending = new Map();
globalThis.__g7RunnerSend = (origin, path, options) => new Promise((resolve) => {
  const id = ++sequence;
...
const actualApi = new URL('../../lib/setup-engine/vaultwarden-api.js?actual', import.meta.url).href;
registerHooks({ resolve(specifier, context, next) {
  let source;
  if (specifier.endsWith('/vaultwarden-api.js') && !context.parentURL?.includes('?actual')) {
    source = `import {createClient as base} from ${JSON.stringify(actualApi)};export * from ${JSON.stringify(actualApi)};export const createClient=(origin,opts={})=>base(origin,{...opts,send:globalThis.__g7RunnerSend});`;
  } else if (specifier.endsWith('/vaultwarden-identity.js')) {
