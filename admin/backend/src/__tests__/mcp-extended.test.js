...
test('the extended catalog is well-formed, unique, and every family is represented', () => {
  assert.equal(MCP_EXT_TOOLS.length, 133);
  ...
  assert.deepEqual(Object.keys(MCP_EXT_TOOL_GROUPS), ['builds', 'project_config', 'lxc_admin', 'edge', 'static_admin', 'admin', 'self_edit']);
  assert.equal(MCP_TOOLS.length, 72 + 133);
