...
test('every /api/setup mutation is admin + fresh sudo behind the global CSRF check, audited; the runner accepts only its kinds and validated parameters', () => {
  ...
  assert.deepEqual([...RUNNER_JOB_KINDS], ['deploy', 'recover_app', 'verify_app', 'probe', 'restore_db', 'restore_snapshot', 'retry_secrets', 'instance_create', 'instance_start', 'instance_stop', 'instance_restart', 'instance_delete', 'snapshot_create', 'snapshot_delete', 'guest_setup', 'config_set', 'device_add', 'device_remove', 'network_pin', 'forward_apply', 'forward_remove', 'egress_set'], 'the two restores and the retry mint are runner jobs since A-13…A-15; the Incus lifecycle and snapshot verbs since A-17; the post-launch guest setup since A-17.7; the guest configuration verbs since A-17.8');
