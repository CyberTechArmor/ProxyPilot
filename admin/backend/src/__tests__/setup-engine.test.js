...
test('store: claimNextJob is compare-and-swap — two claimants, one winner; only listed kinds; the epoch moves', () => {
  ...
  assert.equal(RUNNER_JOB_KINDS.includes('deploy'), false, 'a backend kind is never in the runner\'s list');
  assert.equal(getJob(d, d.prepare(`SELECT id FROM setup_jobs WHERE kind = 'deploy'`).get().id).status, 'queued', 'the deploy job is not the runner\'s to claim');
  assert.equal(won.id, j.id);
  assert.equal(won.status, 'running');
  assert.equal(won.epoch, 1);
  assert.equal(won.owner, RUNNER);
  assert.equal(claimNextJob(d, { owner: 'runner@pp#301:ssss', kinds: [...RUNNER_JOB_KINDS], nowMs: T0 + 3 }), null, 'nothing left to claim');
...
test('verification ladder: states are distinct, a port answering is never "healthy", an unchecked rung caps the state, a failed rung is recovery required', () => {
  ...
  assert.match(healthy.label, /credential not verified/);
  ...
  assert.equal(verified.state, 'credential_verified');
  ...
  assert.equal(badKey.failedAt, 'credential_verified');
  ...
  assert.match(deferred.next, /credential verified was not checked: no data guard recorded/);
});
...
test('retryPlan keeps the approved plan and lists every generated resource for reuse; runner jobs are validated', () => {
  ...
  assert.equal(validateRunnerJob({ kind: 'deploy', app: 'pp-x' }).ok, false);
...
test('the migration text is the store\'s schema, registered as 1000 in db.js', async () => {
  ...
  const deploy = readFileSync(new URL('../mock2/deploy.js', import.meta.url), 'utf8');
  const stopIdx = deploy.indexOf("mark('stopping_app', { app_stopped: true");
  const stopCall = deploy.indexOf('systemctl stop mock2-dev.service');
