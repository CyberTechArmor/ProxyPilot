...
const PAGE = { page: 'Platform Setup', path: '/platform-setup' };
export const HUMAN = Object.freeze({
  save: { ...PAGE, step: 'A. Domains, realm and networks', control: 'Review setup → Save reviewed plan' },
  networks: { ...PAGE, step: 'Restricted networks', control: 'Additional addresses → Save additional addresses' },
  reveal: { ...PAGE, step: 'B. Keycloak, permanent administrator and recovery', section: '1. Initial Keycloak password', control: 'Reveal initial Keycloak password' },
  administrator: { ...PAGE, step: 'B. Keycloak, permanent administrator and recovery', section: '2. Permanent Keycloak administrator', control: 'Create or resume permanent administrator' },
  ...
  setup: { ...PAGE, step: 'the current stage', control: 'the stage card' },
...
function classify(db, reason, { service = null, phase = null } = {}) {
  ...
  if (/read-only Keycloak observer/.test(text)) {
    ...
    if (keycloak?.mode !== 'install') {
      return { code: 'observer_missing', by: 'human', need: `${service || 'The service'} needs the read-only Keycloak observer for its provider, and the provider is not the owned Keycloak. Full Platform installs and manages its own Keycloak.`, where: HUMAN.setup };
    }
    if (obs.present && obs.matches_provider === false) {
      return { code: 'observer_provider_mismatch', by: 'human', need: 'ProxyPilot SSO names a different Keycloak connection than this service. Replacing it is a separately reviewed identity change.', where: HUMAN.setup };
    }
    if (!obs.present) {
  ...
  if (/Custom setup changed the shared service plan|shared service plan changed after this Full Platform revision/.test(text)) {
    return { code: 'shared_plan_changed', by: 'mcp', tool: 'resync_platform_plan',
      need: 'The shared service plan changed after this Full Platform revision recorded it. Resync it (resync_platform_plan, or Platform overview → Resync shared plan): that creates a new Full Platform revision from the saved values, and the next continue writes the shared plan again. No reset is needed.' };
  }
  if (/No private runner address/.test(text)) return { code: 'agent_proxy_address', by: 'human', need: text, where: HUMAN.host };
  if (/confirm the existing Pomerium|external owner must complete|External Keycloak|explicit Custom connection|external connection/.test(text)) return { code: 'external_connection', by: 'human', need: text, where: HUMAN.setup };
  if (/is locked until|belongs to stage/.test(text)) return { code: 'stage_locked', by: 'mcp', tool: 'get_platform_setup', need: text };
...
export function platformSetupView(db) {
  ...
  for (const x of services) {
    if (x.state !== 'failed' || !x.job || x.stage !== s.stage) continue;
  ...
  const view = {
    ...
    // Built-in VPN networks are derived (not editable); additional addresses
...
// Only the CURRENT stage's actions: a later stage is locked until this one is
// verified (get_platform_setup.stages says why), so nothing for it is listed.
function nextActions(db, v, s, full) {
  const out = [];
  const cont = (need) => act('continue', need, 'mcp', { tool: 'continue_platform_setup', args: { revision: v.revision, review_digest: v.review_digest, confirm: true }, stage: v.current_stage });
  if (!full) {
    out.push(act('save_plan', 'Stage A: save the domains, realm and any additional administrator addresses (the VPN networks are included automatically).', 'mcp', { tool: 'save_platform_setup', args: { if_revision: 0 }, human_alternative: HUMAN.save, stage: 'A' }));
    return out;
  }
  ...
  const stage = v.current_stage;
  if (stage === 'A') {
    if (!v.restricted_networks.length) out.push(act('recovery_networks', 'No restricted network is available: the VPN is not enabled here. Save at least one additional administrator address; apply is refused without one.', 'mcp', { tool: 'save_platform_setup', args: { if_revision: v.revision }, stage: 'A' }));
    else out.push(act('apply', `Stage A: apply saved revision ${v.revision}. This starts stage B (Keycloak) only.`, 'mcp', { tool: 'apply_platform_setup', args: { revision: v.revision, review_digest: v.review_digest, confirm: true }, stage: 'A' }));
    return out;
  }
  for (const f of v.failures) {
    out.push(act(`fix_${f.scope}`, `Stage ${stage}: ${f.scope === 'operation' ? 'the setup operation' : f.scope} failed at ${f.reason_code}: ${f.reason} — ${f.remedy}`, f.mcp_can_do ? 'mcp' : 'human',
      { reason_code: f.reason_code, reason_class: f.reason_class, job_id: f.job_id, stage, ...(f.tool ? { tool: f.tool } : {}), ...(f.where ? { where: f.where } : {}) }));
  }
  if (!v.shared_plan_in_sync && !v.failures.some((f) => f.reason_class === 'shared_plan_changed')) {
    const c = classify(db, 'The shared service plan changed after this Full Platform revision recorded it.');
  ...
  for (const x of v.services) if (x.removed && x.stage <= stage) out.push(act(`reinstall_${x.id}`, `${x.name} runtime was explicitly removed; ordinary continue will not undo that.`, 'mcp', { tool: 'manage_platform_service', args: { service: x.id, action: 'reinstall' } }));
  const st = s.state || {};
  const byId = Object.fromEntries(v.services.map((x) => [x.id, x]));
  if (stage === 'B') {
    const identityReady = !!st.identity?.bootstrapRef && byId.keycloak?.state === 'verified';
    if (!identityReady) { if (!v.failures.length) out.push(cont('Stage B: install and verify Keycloak, the managed identity, the SSO record and the recovery route.')); }
    else {
      if (!st.administrator?.masterId) {
        out.push(act('reveal_bootstrap', 'Stage B: reveal the initial Keycloak password (dashboard only, fresh local step-up).', 'human', { where: HUMAN.reveal, stage }));
        out.push(act('administrator', 'Stage B: create the permanent Keycloak administrator — names, email and a permanent password typed by the person.', 'human', { where: HUMAN.administrator, stage }));
      } else {
        out.push(act('passkey', 'Stage B: enroll the application identity\'s discoverable passkey in Keycloak Account Console and confirm its email.', 'human', { where: HUMAN.passkey, stage }));
        out.push(act('access_checks', 'Stage B: link the ProxyPilot account, test SSO login and step-up, and test restricted local recovery from a separate browser.', 'human', { where: HUMAN.accessChecks, stage }));
        out.push(act('retire_bootstrap', 'Stage B: supply a fresh permanent master login (and OTP) and retire the bootstrap account. Stage C unlocks after this.', 'human', { where: HUMAN.retire, stage }));
      }
    }
  }
  if (stage === 'C' || stage === 'D') {
    const ids = STAGES.find((x) => x.id === stage).services;
  ...
  if (stage === 'E') out.push(act('activate_sso', 'Stage E: every service is verified. Activate SSO (dashboard only).', 'human', { where: HUMAN.activate, stage }));
  if (stage === 'complete') out.push(act('done', 'Full Platform setup is complete.', 'mcp', { tool: 'get_platform_setup' }));
  return out;
}
...
export function applyRefusal(db, { kind, revision, reviewToken }) {
  ...
  if (kind === 'continue' && view.current_stage === 'E') return { error: 'Every service is verified. Stage E is activating SSO — a human, step-up-gated dashboard action; there is nothing to continue.', code: 'HUMAN_STEP_REQUIRED', next_action: view.next_actions.find((a) => a.id === 'activate_sso') || null };
  if (kind === 'continue' && view.current_stage === 'complete') return { error: 'Full Platform setup is complete; there is nothing to continue.', code: 'STAGE_COMPLETE' };
...
export function serviceRetryRefusal(db, service) {
  ...
  const locked = stageRefusal(db, service);
  if (locked) return { error: locked, code: 'STAGE_LOCKED' };
  const prior = full.last_job_id && getJob(db, full.last_job_id);
