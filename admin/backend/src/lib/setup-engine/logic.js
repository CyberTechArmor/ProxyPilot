...
export function validateRunnerJob(job) {
  ...
  if (job.kind === 'full_platform_apply') return job.app === 'pp-full-platform' && Number.isInteger(p?.revision) && p.revision > 0 && Object.keys(p).every(k => ['revision', 'operation'].includes(k)) && (p.operation === undefined || ['administrator', 'retire', 'lifecycle', 'reset'].includes(p.operation)) && Object.keys(plan).every(k => k === 'params') ? { ok: true } : { ok: false, reason: 'Full Platform jobs carry only an approved revision reference.' };
