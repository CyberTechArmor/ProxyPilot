import { fail,digest,namesFor,policyFor,humanRoleFor,machineRoleFor,roleFor } from './openbao-logic.js';
...
export async function verifyMachine(r,v,api){const n=namesFor(r);if(!v.roleId)throw fail('Complete the reviewed OpenBao bootstrap handoff first.');
  ...
  if(!a?.client_token||!same([...(a.policies||[])].sort(),[n.machine])||a.lease_duration>120||a.lease_duration<=0)throw fail('Machine authentication did not return the exact limited policy and TTL.');
  const token=a.client_token;
  try{const paths=[`${n.database}/creds/reader`,`${n.database}/creds/not-allowed`,'sys/mounts','sys/policies/acl/default','secret/data/unrelated'];
...
export async function bootstrap(db,r,input,api,{job,verifyDatabase}){const current=await requireReady(api,r),n=namesFor(r),v=secrets(db,r),token=input.bootstrapToken;
  ...
  await verifyDatabase(r,input.databasePassword);job.fence();
  for(const x of owned)if(!x.map?.[x.name+'/'])ok(await api(`/v1/${x.path}/${x.name}`,{method:'POST',token,body:{type:x.type,description:r.credential_ref}}),'Owned mount creation');
  ...
  const dbSpec={plugin_name:'postgresql-database-plugin',allowed_roles:['reader'],connection_details:{connection_url:`postgresql://{{username}}:{{password}}@${r.config.database.host}:${r.config.database.port}/${r.config.database.name}?sslmode=verify-full`,username:r.config.database.username}};
  ...
  if(before.status===404)ok(await api(configPath,{method:'POST',token,body:{plugin_name:dbSpec.plugin_name,allowed_roles:['reader'],connection_url:dbSpec.connection_details.connection_url,username:r.config.database.username,password:input.databasePassword,verify_connection:true}}),'Selected disposable database');
  ...
  if(lookup.status===404)ok(await api(`/v1/auth/${n.approle}/role/workload/custom-secret-id`,{method:'POST',token,body:{secret_id:v.machine,metadata:JSON.stringify({owner:r.credential_ref})}}),'Preserved machine credential');
  ...
  job.fence();putSecrets(db,r,{...v,roleId:id});
...
export async function verifyAccess(r,token,api){const n=namesFor(r);const get=async path=>ok(await api('/v1/'+path,{token}),'Owned access readback')?.data;
  ...
  const config=await get(`${n.database}/config/selected`);assertFields(config,{plugin_name:'postgresql-database-plugin',allowed_roles:['reader']},'Selected database connection');assertFields(config.connection_details,{connection_url:`postgresql://{{username}}:{{password}}@${r.config.database.host}:${r.config.database.port}/${r.config.database.name}?sslmode=verify-full`,username:r.config.database.username},'Selected database target');
