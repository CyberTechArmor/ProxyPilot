...
export async function openbaoRequest(origin,path,{method='GET',token,body,resolve=lookup,request=https.request,local=false,edge=null}={}) {
  ...
  let addresses;
  try{addresses=local?[{address:'127.0.0.1',family:4}]:edge?[{address:edge.address,family:4}]:await Promise.race([resolve(u.hostname,{all:true,family:4}),new Promise((_,reject)=>{const t=setTimeout(()=>reject(Error()),5000);t.unref();})]);}catch{throw fail('OpenBao DNS could not be verified.');}
  if(!addresses.length||(!local&&!edge&&addresses.some(a=>!allowedAddress(a.address))))throw fail('OpenBao DNS points to a blocked special-use address.');
  const data=body===undefined?null:JSON.stringify(body);
  const timeout=u.pathname==='/v1/sys/init'?60000:7000;
  return new Promise((done,reject)=>{
    let bytes=0;const chunks=[];
    const req=(local&&request===https.request?http.request:request)(u,{method,agent:false,timeout,lookup:(_h,o,cb)=>o.all?cb(null,[addresses[0]]):cb(null,addresses[0].address,4),headers:{...(edge?.headers||{}),Accept:'application/json',...(token?{'X-Vault-Token':token} :{}),...(data?{'Content-Type':'application/json','Content-Length':Buffer.byteLength(data)}:{})}},res=>{
...
export function createClient(origin,{send=openbaoRequest,job,local=false,edge=null}={}) {
...
export async function requireReady(api,r){const s=await status(api);if(s.state!=='unsealed')throw fail(s.state==='sealed'?'OpenBao is sealed. Submit the required shares manually after restart, then retry.':s.state==='uninitialized'?'OpenBao is uninitialized. External instances must be initialized by their owner.':'OpenBao is unavailable. Restore connectivity and retry; no service is verified.');
  ...
  if(!s.clusterId)throw fail('OpenBao cluster identity could not be verified.');
