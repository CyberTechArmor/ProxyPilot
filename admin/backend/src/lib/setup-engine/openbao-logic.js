...
export const configSchema=z.object({expectedPlanRevision:z.number().int().positive(),expectedRevision:z.number().int().nonnegative(),connectionId:name,clientId:name,clientSecret:value.optional(),group:z.string().min(1).max(150).regex(/^\/[a-zA-Z0-9_/-]+$/),
  database:z.object({host:z.string().refine(privateIp),port:z.number().int().min(1024).max(65535).default(5432),name:z.string().regex(/^pp_g6_[a-z0-9_]{1,48}$/),username:z.string().regex(/^[a-z][a-z0-9_]{2,62}$/),caPem:z.string().max(32000).refine(publicCa).optional(),disposable:z.literal(true)}).strict(),
