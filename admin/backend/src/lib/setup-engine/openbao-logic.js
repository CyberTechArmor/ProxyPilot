...
export const configSchema=z.object({expectedPlanRevision:z.number().int().positive(),expectedRevision:z.number().int().nonnegative(),connectionId:name,clientId:name,clientSecret:value.optional(),group:z.string().min(1).max(150).regex(/^\/[a-zA-Z0-9_/-]+$/),
  database:z.object({host:z.string().refine(privateIp),port:z.number().int().min(1024).max(65535).default(5432),name:z.string().regex(/^pp_g6_[a-z0-9_]{1,48}$/),username:z.string().regex(/^[a-z][a-z0-9_]{2,62}$/),disposable:z.literal(true)}).strict(),
  pgpKeys:z.array(publicKey).length(3).optional(),rootPgpKey:publicKey.optional(),initialize:z.boolean().default(false),allowedIps:z.array(z.string().refine(v=>isIP(v)===4)).min(1).max(16),reviewed:z.literal(true)}).strict();
...
export const bootstrapSchema=applySchema.extend({bootstrapToken:value,databasePassword:value,revokeBootstrap:z.literal(true)}).strict();
...
export const humanRoleFor=r=>({role_type:'oidc',user_claim:'sub',bound_audiences:[r.config.clientId],bound_claims:{groups:[r.config.group]},bound_claims_type:'string',allowed_redirect_uris:[callbackFor(r)],oidc_scopes:['openid','profile','groups'],token_policies:[namesFor(r).human],token_no_default_policy:true,token_ttl:120,token_max_ttl:120});
