...
import { OPENBAO_ROOT,OPENBAO_IMAGE,OPENBAO_PORT,fail,namesFor,digest } from './openbao-logic.js';
...
export const serverConfig=r=>({ui:true,disable_mlock:true,api_addr:r.config.origin,cluster_addr:'https://127.0.0.1:8201',storage:{raft:{path:'/openbao/file',node_id:namesFor(r).server}},listener:{tcp:{address:'0.0.0.0:8200',cluster_address:'127.0.0.1:8201',tls_disable:true}},log_level:'warn'});
...
export async function ensureRuntime(r,{exec,job,root=OPENBAO_ROOT}={}){const n=namesFor(r),owner=r.credential_ref;
  ...
  if(!prior.server)await call(['create','--name',n.server,...labels,'--restart','unless-stopped','--network',n.network,'--log-driver','none','--memory','512m','--memory-swap','512m','--publish',`127.0.0.1:${OPENBAO_PORT}:8200`,'--mount',`type=volume,source=${n.volume},target=/openbao/file`,'--mount',`type=volume,source=${n.logs},target=/openbao/logs`,'--mount',`type=bind,source=${files.config},target=/openbao/config/server.json,readonly`,OPENBAO_IMAGE,'server']);
  ...
  if(!a.State?.Running)await call(['start',n.server]);
