  ...
  if (isFastBuildMode(mode)) {
    ...
    insertMessage({
      ...
      body: quick
        ...
        : 'MVP build — skipping the rule interview and the gate battery to get a testable first version up fast (the deploy build + health check are the backstop). The standard CRUD rules pack applies as the floor: created records are editable and deletable, statuses fully cyclable, deletes confirm-guarded, no pre-applied filters on load. Run a full Build afterwards to confirm deviations, per-rule tests, and acceptance checks.',
    });
    ...
    proceedToBuild({
      ...
      requestId: request.id, task: { kind: 'feature', difficulty: null }, buildMode: mode, escalate, escalateModel, escalateEffort, escalateThinking,
    })
      .catch((err) => {
...
async function proceedToBuild({ project, instruction, initiatedBy, actingAsAdmin, framework, adminDecisions = '', requestId = null, task = null, buildMode = null, escalate = false, escalateModel = null, escalateEffort = null, escalateThinking = null }) {
  const projectId = Number(project.id);
  ...
  try {
    ...
    result = await startCycle({ project: getProject(projectId), instruction: fullInstruction, initiatedBy, actingAsAdmin, requestId, segment: 'build', task, buildMode, escalate, escalateModel, escalateEffort, escalateThinking });
  } catch (err) {
