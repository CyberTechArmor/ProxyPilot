...
// frameAncestor (optional) — when set (e.g. "https://admin.example.com"), the
// live route rewrites the app's response headers so the ProxyPilot dashboard,
// and ONLY it, may embed the app in an iframe (the build-phase preview). The
// app's own frame-ancestors otherwise refuse framing; see the header_down note.
export function buildMock2SiteBlock({ fqdn, note = '', upstream = null, tlsDecision = null, frameAncestor = null } = {}) {
  const host = q(fqdn);
  ...
  // Preview framing: let the dashboard embed the running app. The generated app
  // sets its own frame-ancestors (constitution §5) and refuses to be framed, so
  // an iframe just shows "refused to connect". We drop X-Frame-Options (it can't
  // express a cross-origin allowance) and relax CSP frame-ancestors to permit
  // ONLY the admin origin — no third-party site can frame the app, so this is a
  // scoped clickjacking allowance, not a removal. Only on the live route, only
  // when the caller supplied the admin origin.
  const validFrame = frameAncestor && /^https?:\/\/[a-z0-9.-]+(:\d{1,5})?$/i.test(frameAncestor) ? frameAncestor : null;
  const proxyBlock = up && validFrame
    ? `reverse_proxy ${up} {
			# Allow ONLY the ProxyPilot dashboard to embed this app (build preview).
			header_down -X-Frame-Options
			header_down Content-Security-Policy "frame-ancestors[^;]*" "frame-ancestors 'self' ${q(validFrame)}"
		}`
    : `reverse_proxy ${up}`;
...
export function buildMock2DomainConfig({ domain, fqdns = [], resolveTls = null, frameAncestor = null } = {}) {
  const decide = typeof resolveTls === 'function' ? resolveTls : () => null;
  const blocks = (fqdns || [])
    .map((f) => (typeof f === 'string' ? { fqdn: f } : f))
    .filter((f) => f && f.fqdn)
    .map((f) => buildMock2SiteBlock({ ...f, tlsDecision: decide(f.fqdn), frameAncestor }))
    .join('\n');
  const header =
...
export async function writeMock2DomainSite(domain, fqdns = []) {
  ...
  const frameAncestor = await mock2FrameAncestor();
  const content = buildMock2DomainConfig({ domain, fqdns, resolveTls, frameAncestor });
...
// The dashboard's own origin, used as the sole allowed frame-ancestor for the
// live preview embed. Lazily imports db.js (native chain) so this module stays
// native-free at IMPORT time. Returns "https://<admin-domain>" or null (null =
// no header rewrite, preview stays blocked but nothing else changes).
async function mock2FrameAncestor() {
  try {
    const { getAdminDomain } = await import('../db.js');
    const host = typeof getAdminDomain === 'function' ? getAdminDomain() : null;
    const clean = host ? String(host).trim().toLowerCase() : '';
