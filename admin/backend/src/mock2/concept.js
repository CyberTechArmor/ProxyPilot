...
const INITIAL_BUILD_INSTRUCTION = `${INITIAL_BUILD_INSTRUCTION_PREFIX}: implement every screen, field, and action it defines on the pinned framework, so the live URL serves the real app in place of the placeholder. The approved mockup is preserved at state/mockups/current.html — READ IT FIRST and reproduce it faithfully: its layout, navigation structure (including patterns like a mobile bottom tab bar), component arrangement, and interaction patterns, screen by screen. The mockup is the visual contract — the app should look and navigate like it, not merely share its colors. state/design.css is NOT just tokens: it carries the mockup's own component CSS, lifted verbatim — its cards, lists, nav bars, chips, buttons, empty states. Open it, take the class names it defines, and put those classes on your elements; style anything genuinely new on var(--...) from the same file, and match state/design-tokens.json (colors, fonts, spacing, radii). A screen of plain elements on the base shell is not a built screen: shipping markup with no styling of its own and none of the approved component classes fails the design-adherence gate, and it produces an app that looks nothing like the mockup you were given. Never fall back to a generic style. Any inventory feature you cannot finish this cycle must be visibly marked "Not built yet" in the UI (a disabled control + badge), never a dead or silently missing element.`;
...
export async function adjustDesignPreset({ presetKey, instruction }) {
  ...
  const res = await callStepTurn('design-doc-adjust', {
    ...
    system, tools: [], transcript: [{ role: 'user', text: user }], timeoutMs: 180000,
      ...
      if (found.ok) {
        ...
        try {
          ...
          const res = await callStepTurn('mockup-screen', {
            ...
            system: stepSystemPrompt('mockup-screen', buildScreenRenderSystemPrompt({ designSystem: boundDesignSystem }), { DESIGN_SYSTEM: boundDesignSystem }),
            tools: [],
            transcript: [{
              role: 'user',
