...
// Rewritten (2026-07 harness redesign, Phase 2) around the seven-point build
// contract; each numbered point below reconciles with what the harness
// actually checks (parity accepts menu placement; the badge is conditional on
// the mockup showing the control; design-adherence scores drift, not breadth).
const INITIAL_BUILD_INSTRUCTION = `${INITIAL_BUILD_INSTRUCTION_PREFIX}: implement every screen, field, and action it defines, so the live URL serves the real app in place of the placeholder. `
  + '1) The platform base app is already in this container — shell, /login, /admin, platform.js, theme.js, service worker, Postgres wiring — built and working. Wire your screens into it; never rebuild, shadow, or duplicate its surfaces. '
