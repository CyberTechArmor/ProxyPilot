...
const signInFor=r=>{const n=namesFor(r);return {url:`${r.config.origin}/ui/`,method:'OIDC',mountPath:n.oidc,role:'mapped',group:r.config.group,tokenMinutes:2,
  scope:r.config.basic?`Read-only proof access: the ${n.prefix}-kv/data/health marker and the owned auth/policy settings. It cannot create or read other secrets, and the token lasts 2 minutes.`:`Read-only proof access: ${n.database}/creds/reader and the owned auth/policy settings; the token lasts 2 minutes.`};};
