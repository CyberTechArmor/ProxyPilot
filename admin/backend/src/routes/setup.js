...
setupRouter.post('/jobs/:id/retry', requireAdmin, requireSudo, (req, res) => {
  ...
  if (['keycloak_setup', 'configure_keycloak_route'].includes(prior.kind)) return res.status(409).json({ error: 'Review the saved Keycloak plan and retry from Platform Setup; its revision is required.' });
