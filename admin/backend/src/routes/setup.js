...
setupRouter.post('/jobs/:id/retry', requireAdmin, requireSudo, (req, res) => {
  ...
  if (['keycloak_setup', 'configure_keycloak_route', 'infisical_apply', 'configure_infisical_route', 'openbao_apply', 'configure_openbao_route', 'openbao_operator'].includes(prior.kind)) return res.status(409).json({ error: 'Review the saved platform service plan and retry from Platform Setup; its revision is required.' });
