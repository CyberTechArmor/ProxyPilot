      ...
      return <div className="space-y-4">
        ...
        {isCurrent && st.identity?.bootstrapRef && <>
          ...
          <fieldset disabled={locked || !st.administrator?.masterId} className="space-y-3 rounded-lg border p-4 min-w-0"><legend className="px-1 font-semibold">5. Retire the bootstrap account</legend>
            <p className="text-sm">After the checks above pass: a fresh login as the permanent master administrator (password above, and OTP if enrolled) proves administration, then the temporary account is removed. Stage C unlocks after this.</p>
            <Button variant="outline" className="min-h-11 h-auto whitespace-normal w-full sm:w-auto" onClick={() => run('retire', async () => { try { await api.fullPlatformAdministrator({ ...administrator, otp: administrator.otp || undefined, revision: data.revision, action: 'verify_and_retire', reviewed: true }); setNotice('Verification and retirement queued.'); await load(); } finally { setAdministrator(p => ({ ...p, password: '', otp: '' })); } })}>Verify administration and retire bootstrap</Button>
          </fieldset>
        </>}
      </div>;
    }
