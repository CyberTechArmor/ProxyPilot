...
import InfisicalAdministrator from '@/components/InfisicalAdministrator';
...
export default function FullPlatformSetup() {
  ...
  // The review panel renders below all stages; bring it to the operator.
  useEffect(() => { if (lifecycle) lifecycleRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }); }, [lifecycle]);
    ...
    if (stage.id === 'C' || stage.id === 'D') {
      ...
      return <div className="space-y-4">
        ...
        {isCurrent && stage.id === 'D' && <>
          <details className="rounded-lg border p-4"><summary className="min-h-11 cursor-pointer flex items-center font-medium">Infisical administrator</summary><InfisicalAdministrator data={data} administrator={administrator} locked={locked} run={run} load={load} setNotice={setNotice} /></details>
          <details className="rounded-lg border p-4"><summary className="min-h-11 cursor-pointer flex items-center font-medium">OpenBao recovery custody</summary><div className="pt-3"><OpenBaoSetup revision={data.planRevision} fullRevision={data.revision} dirty={dirty} choice={config.services.openbao} connections={connections} managed /></div></details>
  ...
  return <div id="full-platform-setup" className="space-y-6 min-w-0 max-w-6xl shrink-0 pb-6">
    ...
    <PlatformOverview />
    {data?.session && !data.session.localActions && <div role="note" className="flex gap-2 rounded-lg border border-amber-500/50 bg-amber-500/10 p-4 text-sm break-words"><AlertTriangle className="h-4 w-4 mt-0.5 shrink-0 text-amber-500" aria-hidden="true" /><p>{data.session.method === 'oidc' ? <>This browser is signed in through Keycloak (from the SSO login test), so setup actions that need a local sign-in will be refused: entering passwords, runtime repair/reinstall/remove and reset. Sign out and sign back in at <strong>{data.session.origin}</strong> with your local password + authenticator code or ProxyPilot passkey, then continue. Stage E asks for a fresh SSO test again before activation.</> : <>This browser session cannot do setup actions that need a local sign-in. Sign out and sign back in at <strong>{data.session.origin}</strong> with your local password + authenticator code or ProxyPilot passkey.</>}</p></div>}
    {error && <p role="alert" className="rounded-lg border border-destructive p-4 text-destructive break-words">{error}</p>}
    {error && <div role="alert" className="fixed inset-x-4 bottom-4 z-50 sm:left-auto sm:max-w-xl rounded-lg border border-destructive bg-background p-4 shadow-lg space-y-2"><p className="text-destructive text-sm break-words">{error}</p><Button variant="outline" className="min-h-11" onClick={() => setError('')}>Dismiss</Button></div>}
