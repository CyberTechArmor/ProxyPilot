...
export default function InfisicalAdministrator({ data, administrator, locked, run, load, setNotice }) {
  ...
  const submit = (e, generate) => {
    e.preventDefault();
    const f = new FormData(e.currentTarget), input = { revision: data.revision, email: f.get('email'), ...(generate ? { generate: true } : { password: f.get('password') }), reviewed: true };
  ...
  return <div className="pt-3 space-y-4 min-w-0">
    ...
    {vault.generated && vault.location && <p className="text-sm rounded-lg border p-3 break-words">The password was generated and is kept in OpenBao: <code className="break-all">{vault.location.engine}</code> → <code className="break-all">{vault.location.path}</code>. See “Use your platform” below for how to open it.</p>}
    {vault.canGenerate
    ...
    <details className="rounded-lg border p-3"><summary className="min-h-11 cursor-pointer flex items-center text-sm font-medium">Choose a password yourself</summary>
      <form className="pt-3 space-y-3" onSubmit={e => submit(e, false)}>
        <p className="text-sm">Choose it once; retries use the same account. It is removed from the protected handoff after use or expiry, so keep it in Vaultwarden or OpenBao yourself.</p>
