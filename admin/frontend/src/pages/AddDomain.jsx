...
import {
  Globe, KeyRound, Loader2, ShieldCheck, Eye, EyeOff, CheckCircle2,
  XCircle, AlertTriangle, Lock, Sparkles,
} from 'lucide-react';

// Public self-service page: provision a domain on the Caddy reverse proxy
// with an automatic Let's Encrypt certificate. Standalone route (outside
// the dashboard shell) gated by a ProxyPilot access API key — NOT a
// dashboard session. All requests go straight to /api/domains/provision/*
// with the key in X-API-Key and no cookies; the key lives only in
// component state (never persisted).
//
// The Cloudflare token field is a different credential entirely (Caddy's
// DNS-01 challenge). It is shown only when the resolved method is DNS-01,
// masked, optional (blank = the server's global token), and never echoed
// back by the server.
...
async function providerFetch(path, apiKey, { method = 'GET', body } = {}) {
  ...
  if (!res.ok) {
    const err = new Error(data.error || (Array.isArray(data.errors) && data.errors[0]) || `Request failed (${res.status})`);
...
export default function AddDomain() {
  ...
  const [resolved, setResolved] = useState(null); // { method, reason } | { error }
  ...
  const unlock = async (e) => {
    ...
    setGateError('');
    try {
      const r = await providerFetch('/provision/verify-key', key, { method: 'POST', body: {} });
      setUnlocked(true);
      setKeyName(r.keyName || '');
      setGlobalTokenAvailable(!!r.globalTokenAvailable);
    } catch (err) {
      setGateError(err.status === 401 ? 'That API key was not accepted. Check it and try again.' : err.message);
    } finally {
      setGateBusy(false);
    }
  };

  // Live method resolution: debounce while the user types so the resolved
  ...
  const resolveNow = useCallback(async () => {
    ...
    try {
      const r = await providerFetch('/provision/resolve', apiKey.trim(), {
        method: 'POST',
        body: {
          domain: d,
  ...
  }, [unlocked, apiKey, domain, upstream, method, wildcard, acmeEmail, cfToken]);
  ...
  const startPolling = useCallback((d) => {
    ...
    const tick = async () => {
      try {
        const s = await providerFetch(`/provision/${encodeURIComponent(d)}/status`, key);
        setIssuance(s);
  ...
  const submit = async (e) => {
    ...
    setIssuance(null);
    try {
      const r = await providerFetch('/provision', apiKey.trim(), {
        method: 'POST',
        body: {
          domain: domain.trim(),
  ...
  return (
    <div className="min-h-screen bg-background flex items-start justify-center px-4 py-8 sm:py-14">
        ...
        {!unlocked ? (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Enter your ProxyPilot access API key to use this page. (This is the key an
                administrator generated for you — not a Cloudflare token.)
              </CardDescription>
            </CardHeader>
            <CardContent>
              <form onSubmit={unlock} className="space-y-4">
                ...
                {gateError && (
                  <p className="text-sm text-red-500 flex items-start gap-2">
                    <AlertTriangle className="h-4 w-4 mt-0.5 shrink-0" /> {gateError}
                  </p>
                )}
                <Button type="submit" disabled={gateBusy || !apiKey.trim()} className="w-full min-h-[44px]">
                  {gateBusy ? <Loader2 className="h-4 w-4 animate-spin" /> : <KeyRound className="h-4 w-4" />}
          ...
          <>
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              Unlocked{keyName ? <> with key <span className="font-medium text-foreground">{keyName}</span></> : null}
            </div>
