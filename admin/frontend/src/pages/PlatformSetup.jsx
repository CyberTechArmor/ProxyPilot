  ...
  return <div className="space-y-6 min-w-0 max-w-6xl">
    ...
    {!data ? <p role="status">{busy ? 'Loading platform plan…' : 'The platform plan could not be loaded. Try reopening it.'}</p> : <>
      <div className="rounded-lg border bg-muted/30 p-4 space-y-2 text-sm">
        ...
        <p>Your current login and running applications stay in place. Service installation and login activation are not available yet.</p>
      </div>
      ...
      {stage === 'choose' ? <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">{data.services.map((service) => {
        ...
        return <Card key={service.id} className="min-w-0"><CardHeader><CardTitle>{service.name}</CardTitle><CardDescription>{service.description}</CardDescription></CardHeader><CardContent className="space-y-4">
          ...
          <p className="text-xs text-muted-foreground">Verified installed state: not checked. A service-specific adapter is required.</p>
        </CardContent></Card>;
        ...
        <ul className="divide-y">{data.services.map((service) => <li key={service.id} className="py-3 min-w-0 break-words"><p className="font-medium">{service.name} · {modeLabel[choices[service.id].mode]}</p>{choices[service.id].mode !== 'skip' && <><p className="text-sm break-all">{choices[service.id].url || 'Endpoint required'}</p>{service.id === 'infisical' && <p className="text-sm break-all">Agent Proxy: {choices.infisical.agentProxyUrl || 'Endpoint required'}</p>}</>}<p className="text-xs text-muted-foreground">Installation / connection unverified</p></li>)}</ul>
        <div className="rounded-lg border p-3 text-sm">All selected services still need their installation or connection adapters. Pomerium needs a verified identity provider. TLS, backup and recovery prerequisites remain unverified.</div>
        <div className="flex flex-col sm:flex-row flex-wrap gap-2"><Button className="min-h-11 bg-foreground text-background hover:bg-foreground/90" disabled={!!busy} onClick={save}>{busy === 'save' ? 'Saving plan…' : 'Save reviewed plan'}</Button><Button variant="outline" className="min-h-11" disabled>Install services — unavailable</Button><Button variant="outline" className="min-h-11" disabled>Activate login — unavailable</Button></div>
