...
function renderClientConfig({ peerPrivateKey, peerIp, scope, cfg }) {
  // No DNS = ... line: AllowedIPs is 10.100.0.0/24 for every scope,
  // so a DNS server outside that range can't be reached through the
  // tunnel anyway (the client OS hijacks DNS to the tunnel adapter,
  // queries dead-end, name resolution silently fails). Operators who
  // run a resolver on the VPN subnet itself (10.100.0.x) can edit
  // the rendered config by hand.
  const lines = [
