...
function allowedIpsFor(_scope) {
  // Every peer routes only the VPN subnet — operator's regular
  // internet stays direct on every scope. The scope axis is enforced
  // at the firewall layer (admin/full = unrestricted reachability
  // inside 10.100.0.0/24, services = restricted to tagged services);
  // it is NOT a client-side routing axis.
  //
  // Historical note: `full` previously emitted 0.0.0.0/0, ::/0 to
  // tunnel everything. That required a working MASQUERADE/forwarding
  // path off the host's egress interface, which proved unreliable in
  // practice (operator-reported breakage). Routing only 10.100.0.0/24
  // gives the same VPN-subnet reachability without depending on the
  // host's NAT/forwarding posture for arbitrary internet egress.
...
export function renderClientConfig({ peerPrivateKey, peerIp, scope, cfg, mtu }) {
  ...
  // the platform hostnames and the operator's extra domains with 10.100.0.1
  // — so those requests go through the tunnel and Caddy sees the peer's VPN
  // address — and forwards every other query to the host's resolvers. Only
  // emitted when the configured DNS address is inside the VPN subnet (a
  // resolver outside it could not be reached through the split tunnel).
  ...
  const lines = [
    ...
    `Address = ${peerIp}/32`,
    ...(/^10\.100\.0\.\d{1,3}$/.test(dns) ? [`DNS = ${dns}`] : []),
    `MTU = ${m}`,
  ];
  lines.push(
    '',
