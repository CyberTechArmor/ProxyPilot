...
func DefaultRegistry() *Registry {
	r := NewRegistry()
	r.Register("agent.ping", noParams(AgentPing))
	r.Register("caddy.adapt", CaddyAdapt)
	r.Register("caddy.reload", CaddyReload)
	r.Register("caddy.fmt", CaddyFmt)
	r.Register("caddy.list_modules", noParams(CaddyListModules))
	r.Register("caddy.version", noParams(CaddyVersion))
	r.Register("storage.list_disks", StorageListDisks)
	r.Register("storage.zpool_status", noParams(StorageZpoolStatus))
	r.Register("storage.zfs_list", noParams(StorageZfsList))
	r.Register("security.cve_2026_31431.check", noParams(SecurityCVE202631431Check))
