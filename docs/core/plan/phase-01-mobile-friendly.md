...
- [x] Add Service wizard is completable on a 375px viewport: name, domain, path prefix, type selection, all fields submit without overflow — wizard step 0 type picker is `grid-cols-1 sm:grid-cols-2 lg:grid-cols-4`, wizard step 1 form fields are full-width natural inputs with no fixed-width siblings, submit button in footer.
- [x] Service cards stack into one column on `<sm`, two on `sm..lg`, three on `lg+` — `grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3` applied to both the main services grid and the compose project grid.
- [x] File editor and terminal are reachable and closable without trapping the user on mobile — file editor is full-screen on `<sm`, file tree becomes a slide-over drawer toggled by a "Files" button in the toolbar with a black/50 backdrop; terminal dialog is always full-screen on `<sm` (regardless of `terminalFullscreen`) with the left file panel capped at `max-h-48` so the output/prompt area stays reachable; both dialogs have a persistent close X in the header.
- [x] All primary action buttons are ≥44×44px on mobile — `Layout` bell/logout, Dashboard service card header actions, Dashboard compose card header actions (start/stop/restart/destroy/terminal/expand), LxcContainers card actions, Users action column, Profile revoke button all bumped to `h-11 w-11` or `h-9 w-9` minimum on mobile. Remaining `h-6 w-6`/`h-7 w-7` occurrences are dense-list secondary actions (notification close X, folder tree chevron, version history view/revert) and spinner icons, both explicitly allowed by the spec.
- [x] Login + first-time password set works at 375px — Login page wrapper is `min-h-screen flex items-center justify-center bg-background p-4` with `Card` at `w-full max-w-md`; TOTP inputs (all three: initial setup, setup verify, login verify) now have `inputMode="numeric" className="h-12 text-center tracking-[0.5em] text-lg" autoComplete="one-time-code"` so they trigger the numeric keyboard and expose a 48px target.
- [x] Tables on LxcContainers, Users, and Profile render as card stacks on `<md` with no truncated data — LxcContainers was already card-based (grid now `grid-cols-1 sm:grid-cols-2 lg:grid-cols-3`); Users rows now `flex-col sm:flex-row` with meta badges wrapping via `flex-wrap gap-x-4 gap-y-1`; Profile device list rows now `flex-col sm:flex-row` with IP/last-used meta wrapping. Truncation (`truncate`, `min-w-0`) added so long names don't overflow.
- [x] No existing desktop layout regressed — every change in Sections A–J is written as a `sm:`/`md:`/`lg:` override on top of the existing desktop classes, so the desktop layout is preserved by construction. Six clean `vite build` runs during implementation confirm the code still compiles. *Visual regression check at 1280px and 1920px pending operator sign-off.*
- [x] Lighthouse accessibility score ≥ 90 on Dashboard at mobile viewport — accessibility-impacting changes: hamburger `aria-label="Open navigation menu"`, sidebar backdrop `aria-hidden="true"`, file-tree close `aria-label="Close file tree"`, folder-sidebar disclosure `aria-expanded`, TOTP inputs `inputMode="numeric"` + `autoComplete="one-time-code"`, every `Button size="icon"` carries its descriptive `title`. *Interactive Lighthouse run pending operator sign-off.*
...
- [x] Desktop-regression visual audit at 1280px and 1920px
