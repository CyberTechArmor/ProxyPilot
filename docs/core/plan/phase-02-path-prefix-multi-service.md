...
- [x] Update endpoint uniqueness check (admin/backend/src/routes/services.js:901)
      — Moved the check after the `updatedData` merge (so `updatedData.pathPrefix` is already normalized), broadened the trigger to fire whenever domain OR path_prefix changes (previously only domain), and rewrote the query as `SELECT id FROM services WHERE domain = ? AND path_prefix = ? AND id != ?`. Same new error message as the create endpoint. **Verified:** Express integration test — seeded two services on `mul.test` (`/` + `/api`); PUT `/api → /` returns 400 with the collision error; PUT `/api → /v2` returns 200 with the DB updated; PUT rename with no prefix/domain change returns 200 without a false-positive; PUT domain change to a fresh domain returns 200.

- [x] SSL-consistency validation on create + update (admin/backend/src/routes/services.js:679 and :941)
      — Both endpoints reject any mutation that would put a service whose `sslEnabled` or `forceHttps` differs from an existing sibling on the same domain. The create check runs after uniqueness with `SELECT ssl_enabled, force_https FROM services WHERE domain = ? LIMIT 1`; the update check runs with `WHERE domain = ? AND id != ? LIMIT 1` so it cannot match the row being updated. **Verified:** Express integration test — created root `ssl.test/` with SSL on (201); sibling `/api` with SSL off rejected with the new error message; sibling `/api` with SSL on accepted (201); PUT existing sibling to `sslEnabled: false` rejected; PUT existing sibling to `forceHttps: false` rejected; PUT existing sibling to a port-only change accepted (200, no false-positive when SSL stays consistent).
...
- [x] Info banner in Add Service wizard (admin/frontend/src/pages/Dashboard.jsx:2877, directly under the Path Prefix Input)
      — Renders a `data-testid="existing-prefixes-banner"` div with classes `rounded-md border border-blue-500/30 bg-blue-500/10 p-3 text-sm text-blue-600 dark:text-blue-300` when `existingPrefixesForDomain.length > 0`. Text reads `Domain already in use. Existing path prefixes: <code>{joined}</code>. Choose a different prefix to add a second service to this domain.` **Verified:** seeded backend with two services on `mul.test` (`/` + `/api`), drove puppeteer through Login auto-bypass (token in localStorage) → Add Service → Static tile → typed `mul.test` at both 360×640 and 1280×800; banner visible at both viewports with the correct text including `/, /api`, no JS errors, `scrollWidth === clientWidth`. Also verified typing `fresh.test` hides the banner (memo recompute).

- [x] Client-side collision guard in `handleAddService` (admin/frontend/src/pages/Dashboard.jsx:779)
      — Before `api.createService(submitData)` runs, a small inline `normalize()` mirrors the backend `normalizePathPrefix` (trim, leading-`/`, strip trailing slashes, default `/`). If the normalized prefix is already in `existingPrefixesForDomain.map(normalize)`, the handler toasts the same error string as the backend (`Domain + path prefix combination already exists`), clears the submitting state, and returns. **Verified:** puppeteer test at 360×640 and 1280×800 — opened wizard, picked Static, typed `mul.test` + `/` (the existing root tuple), clicked Create Service: dialog stays open, toast shows the exact backend error string, no JS errors, no horizontal scroll.

- [x] Domain-grouped header row in the services grid (admin/frontend/src/pages/Dashboard.jsx:3429 + new `renderServiceCard` helper)
      — Extracted the per-card JSX into a `renderServiceCard(service)` helper inside the component (byte-for-byte identical to the pre-Phase-2 inline `<Card>...</Card>`), then replaced the inline `.map` with an inline IIFE that — when `sortBy === 'favorite'` — groups `filteredServices` by domain (preserving inherited per-group order) and emits a `data-testid="domain-group-header"` row above each group with 2+ services. The header spans `col-span-1 sm:col-span-2 lg:col-span-3` so it occupies a full grid row at every breakpoint. Single-service domains stay headerless. **Verified:** puppeteer test at 360×640 and 1280×800 — clicked the Services tab on a Dashboard seeded with `mul.test` (2 services) + `other.test` (1 service); confirmed exactly one header `"2 services on mul.test"`, 3 cards rendered, no `other.test` header, no JS errors, no horizontal scroll.

- [x] Delete confirmation sibling warning (admin/frontend/src/pages/Dashboard.jsx:3884)
      — Switched the `DialogDescription` to `asChild` so it can wrap a small block of paragraphs, then injected an inline IIFE that computes `siblingsCount` from the live `services` array and renders an amber `data-testid="delete-sibling-warning"` paragraph when `siblingsCount > 0`. Singular/plural grammar is handled. **Verified:** puppeteer test at 360×640 and 1280×800 — opened the delete dialog for the `Front` service on `mul.test` (which has a `/api` sibling), confirmed the amber warning reads `"This will leave 1 other service running on mul.test."`; opened the delete dialog for the `Other` service on the single-service `other.test` domain, confirmed no warning element is rendered. No JS errors, no horizontal scroll.
