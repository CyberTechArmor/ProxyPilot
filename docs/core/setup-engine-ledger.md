...
| A2d: in a real guest, a detached (`setsid`) writer started by a deploy script is stopped by the next deploy's reap through its `mock2-deploy-<job>-*` scope (journal shows the `systemctl kill`) and the mock2-dev service is untouched; a guest with systemd masked and no writable cgroup makes the deploy refuse with `containment_unavailable`; a fresh install reads `runner-required` in `.env` only after the unit is active, and `update.sh` with the unit masked prints the red warning and changes nothing; a second deploy submitted before the first's follow-up ran leaves the first's rung `superseded`; a deploy failed after the stop shows the post-failure verification's outcome | outstanding — the sandbox proved the raw cgroup mechanisms with real processes and everything else against scripted guests |
