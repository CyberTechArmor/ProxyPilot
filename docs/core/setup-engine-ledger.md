...
| Phase F, and the operations still on the container's pivot | With the runner live, the deploy, both restores and the retry-path mint no longer run under the container's nsenter pivot. Still on it: Incus lifecycle and the snapshots other than the restore, the component pre-install, Caddy, storage, migration, the workspace terminal. Each is a candidate for the same treatment (a job kind + the runner); `privileged: true`, `pid: host` and the Docker socket stay until they are all moved (master-spec Phase F) |
...
database restore, the snapshot restore and the retry-path mint. They
removed nothing from the container itself: `privileged: true`,
`pid: host`, the Docker socket and the nsenter pivot remain for Incus
lifecycle and the other snapshot verbs, the component pre-install, Caddy,
storage, migration and the workspace terminal. Until the
