...
**Milestone A estimate: ≈ 88 %** (from 85 % on acceptance of the restores
slice; the platform ledger carries the basis: A-17 is about ten points over
nine groups and this first group — five verbs on two surfaces plus the
interruption model and identity binding the later groups reuse — is about
three of them). What keeps the milestone short of complete: A-16 (the MCP
observe verbs), the eight remaining A-17 groups named in the platform
ledger, and Phase F. Live-host acceptance is separate.
...
| Phase F, and the operations still on the container's pivot | With the runner live, the deploy, both restores, the retry-path mint and the Incus lifecycle / snapshot verbs of the dashboard and MCP no longer run under the container's nsenter pivot. Still on it, by group (platform ledger A-17.7…A-17.14): the post-launch configuration and post-start NAT / DNS fix-ups; the guest configuration verbs and their pre-mutation `takeLxcSnapshot`; rename, clone, import / export and the transports' temp instances; project provisioning and the idle sweep; the component pre-install; Caddy; storage and migration transports; the terminal, `run_lxc_command`, the file tools, host service control and every read. Each is a candidate for the same treatment (a job kind + the runner, or the unprivileged agent); `privileged: true`, `pid: host` and the Docker socket stay until they are all moved (master-spec Phase F) |
...
## Overall new-platform repository work: ≈ 39 % (from 38 % on acceptance of the restores slice)

Basis for the increase: milestone A moved three points and is one of four
milestones of unequal size; the weighting keeps the runner / engine and the
...
`pid: host`, the Docker socket and the nsenter pivot remain for the
post-launch configuration and post-start fix-ups, the guest configuration
verbs and their pre-mutation snapshots, rename / clone / import / export
and the transports, project provisioning and the idle sweep, the component
pre-install, Caddy, storage, migration, the workspace terminal and every
read (platform ledger A-17.7…A-17.14 name each by file and verb). Until the
...
| A9 | after the lifecycle group (this branch, on `main@fb7a89d`) | 2895 | 2883 | 5 | 7 | +21 tests (20 in `setup-lifecycle.test.js`, one ratchet in `immediate-repairs.test.js`), all passing; the base `fb7a89d` measured **2874 / 2862 / 5 / 7 in this sandbox, sequentially, with the same five failures**: the four native-module files (`docs/known-issues.md`) and the cgroup2 form of the containment real-process regression, which fails under the FULL parallel `node --test` run on this host on base and branch alike and passes when `setup-deploy-closeout.test.js` runs alone (13 / 0 / 1 skip) — an environment condition, identical on both sides (platform ledger R-022). This sandbox had no `node_modules`; they were installed with `--ignore-scripts` (R-015), so the earlier rows' 4-failure count is not directly comparable to this row's 5 |
