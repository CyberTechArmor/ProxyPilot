...
| `config` | `[{ "key": "AUTH_JWT_SECRET", "secret": true, "required": true, "generate": true, "default": null, "description": "…" }]` | Structured env vars. Non-secret defaults are merged into the project `.env` at install. A secret the component **owns** (a signing key, an at-rest key) carries `generate: true`: the platform mints a random value once per project into the container's `/etc/environment` at install and on every deploy, never overwriting one already there, and the deploy refuses to start production mode while one is missing. A third-party credential (`secret: true` without `generate`) is never written — it surfaces on the operator verification checklist. A minted secret that **replaces a key data was encrypted under** also declares `requires_marker: { "path", "contains", "built"? }`: it is minted only when that source file in the project carries that substring (the code that can migrate the data) and, when `built` names the compiled artifact and it exists, that carries it too; otherwise deferred and reported — the installer never overwrites a file an app already has, so a project generated before the migration code existed keeps its current value. A minted secret that encrypts stored rows also declares `protects: { "table", "secret_column", "nonce_column", "filter", "legacy_default" }`: before changing that key the platform reads those rows from the app's database and classifies them — fresh storage, data the component's bridge can migrate, or an unknown key / unreadable probe, which defers. A non-secret entry may carry `fresh_value`: written into the container environment on the component's **first** install into a project, and only when every `protects` probe confirms fresh storage (new files over an existing database do not count), never on a reinstall — e.g. an empty legacy-key list for a fresh app. Max 48. |
