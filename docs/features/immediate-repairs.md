...
| `restore_project_db` (MCP) | **Covered by the container lock, as a runner job since A-13** (`docs/features/setup-engine.md` § Restores): the app's lease is taken by the job, the restore is **refused before any change** while a deploy or another restore holds it (the reply names the holder), the unit is stopped for the restore, and the job holds the lease for its own lifetime, so a deploy arriving mid-restore queues behind it. It is still the one path that brings rows under an *older* key into an app whose bridge is already closed: after restoring a dump into a fresh-provisioned app, re-open the bridge (`set_project_env` `AUTH_LEGACY_MASTER_SECRETS` to the key those rows used), redeploy, and read `MASTERKEY_ROWS` |
| `restore_snapshot` (MCP and the dashboard, whole-container snapshot) | **Covered by the container lock, as a runner job since A-14**, the same way: refused while a deploy or restore holds the container, held for the restore's lifetime |
| The retry path's secret mint (`runner.js`, before "Retry deploy") | **Covered, as a runner job since A-15** (`retry_secrets`): it holds the lease while it reads and rewrites `/etc/environment` |
