...
| LXC/Incus lifecycle | `create_lxc_container`, `control_lxc_container`, `set_lxc_config`, `set_lxc_network`, `snapshot_lxc_container`, `lxc_file_diff`, `restore_lxc_file` | `create_lxc_container` also accepts `vm:true` to launch an Incus VM; it forces Docker readiness off, `security.nesting=false` and `security.guestapi=false`, and reports the instance type. Its default profile/bridge and best-effort disk override do **not** establish a supervised-agent isolation boundary. Every mutation requires `confirm: true` and snapshots first (since A-17.8 `set_lxc_config` and `set_lxc_network` run as setup-engine jobs — `config_set` / `network_pin` — whose runner takes that snapshot under the guest's lease and reads the key or the reservation back before reporting; `docs/features/setup-engine.md` § "The guest configuration verbs"); this original set has no delete verb and no force-kill (deletion lives in the extended surface behind an export precondition and a one-time confirmation token — see below). `set_lxc_config` writes only the keys in `lib/mcp-policy/lxc-config-allowlist.json`; `security.privileged=true` additionally demands `acknowledge_risk: true` and carries the container-root-is-host-root warning. `set_lxc_network` pins a guest's IPv4 (static DHCP reservation) so a lease renewal can't silently 502 a route. `lxc_file_diff`/`restore_lxc_file` complete the `.old` backup story (restore swaps, so it's reversible). |
...
| ProxyPilot self-update | `check_proxypilot_update`, `get_proxypilot_update_status`, `run_proxypilot_update` | Update ProxyPilot itself from chat. The check reports the running version vs the latest GitHub release and commit, the Mock2 standards seed vs the live site, and the host checkout facts (branch, sha, uncommitted changes); the run tool requires `confirm: true`, refuses on its own when the checkout is dirty, a run is live or the host agent is unreachable, and never discards local changes. What runs is `update.sh --yes` through the root-owned runner (`deploy/proxypilot-update.service`) — the API is unreachable for ~1–2 minutes mid-run; poll the status tool through it. Policy: `lib/mcp-policy/self-update-allowlist.json` (`enabled: false` turns the run tool off). See `docs/features/self-update.md`. |
...
## The extended surface (193 tools, 2026-09)
...
| Storage (ZFS) | `list_disks` (SMART, signatures, `os: true`) · `zpool_status` · `zfs_list` · `list_zfs_snapshots` · `create_zpool` · `set_managed_pool` · `create_dataset` / `set_dataset_props` / `destroy_dataset` · `zfs_snapshot` / `zfs_rollback` / `destroy_zfs_snapshot` · `replace_disk` · `zpool_scrub` · `import_pool` / `export_pool` · `set_incus_storage_pool` · `set_default_storage_pool` (which pool NEW guests land in — moves nothing) · `move_guest_storage` · `get_backup_policy` / `set_backup_policy` · `replication_status` / `set_replication_target` / `run_replication` · `restore_guest_from_snapshot` · `rollback_guest_dataset` · `storage_freshness` · `storage_toolchain` | A different confirm shape: every write is **plan/confirm** — `dry_run: true` returns the exact command plan and a `plan_token` (sha256 of the plan); the real call carries `confirm: true` and that token; a changed plan invalidates it. Devices by `/dev/disk/by-id` only; the OS device has no override. Flag `mcp.storage`; the destroy / rollback verbs also sit behind `mcp.destructive`. `docs/features/storage.md`. |
| Migration | `create_migration` · `get_migration` · `list_migrations` · `approve_migration` · `migration_cutover` · `cancel_migration` · `migration_preflight` · `enable_incus_listener` · `list_migration_tokens` · `revoke_migration_token` · `cleanup_migration` | Adopt a running application from another server, VM or container. `create_migration` returns ONE line for the operator to run as root on the SOURCE; the agent sends an inventory manifest (env PATHS and KEY NAMES, never values — a manifest carrying one is refused) and then WAITS: nothing is copied until `approve_migration`. The imported guest comes up with the default-deny egress fence and no route, and `migration_cutover` records each checklist step and each egress decision. Flag `mcp.migration`. `migration_preflight` is the readiness report (agent builds, callback URL, TLS pin, Incus listener, and which transports can run now); `enable_incus_listener` turns the Incus network listener on at the **bridge gateway** by default and refuses a bind on every interface unless `allow_public` says the source is on the internet. `list_migration_tokens` / `revoke_migration_token` are the token's life: it ends when the migration does, not on a clock, and revoking is the undo for a command pasted somewhere it should not have been. `cleanup_migration` throws away what a FINISHED migration left — the guest it created, the record, or both — and refuses a live migration, an adopted guest and a guest serving a route (`mcp.destructive`). `docs/features/migration.md`. |
| Platform Setup (Full Platform) | `get_platform_setup` · `get_platform_service` · `list_platform_jobs` / `get_platform_job` · `platform_preflight` · `verify_platform_service` · `get_platform_service_logs` · `save_platform_setup` · `apply_platform_setup` · `continue_platform_setup` · `manage_platform_service` (token) · `control_platform_container` (token) · `set_platform_restricted_networks` (token) · `resync_platform_plan` · `recover_keycloak_bootstrap` (token) · `reset_platform_setup` (token) | Thin over the dashboard's own stores (`saveFullPlatform`, `applyFullPlatform`, `applyService`, `lifecycleReview`/`queueLifecycle`, `resetReview`/`queueReset`), so refusals and messages are the dashboard's. `get_platform_setup` returns the step statuses, failures with `reason_code` and plain reason, the G3 observer status, and ordered `next_actions` — each `mcp_can_do` with the tool and arguments, or a human step with its page/step/control. Apply/continue carry `revision` + `review_digest`; `continue_platform_setup({ service })` retries one adapter reusing its stored encrypted inputs and refuses a retry that would fail on a missing observer. Runtime actions and reset use the one-time token bound to the review digest they previewed. Master flag `mcp.platform` gates **every** tool here, readers included, and is **human-only** (`set_feature_flag` refuses it both ways; an administrator flips it on the Platform Setup page, audited; off on new installs, migration 915 kept existing installs on). The runtime/network/resync/recovery/reset tools also need `mcp.destructive` (verify and logs do not); `reset_platform_setup({ purge_data: true })` also `mcp.platform.purge` (**off by default**). Never callable: reveal of the initial Keycloak password, administrator/recovery password entry, bootstrap retirement, SSO activation, any secret input. Every output is redacted and additionally scrubbed of every protected value the install holds. Jobs record `requested_by: "mcp:<key id>"`, `via: "mcp"`. `docs/features/full-platform-setup.md`. |
...
- **Snapshots use the subcommand CLI.** Current Incus spells snapshot
  ...
  Since A-17.8 the config, network, resources and device tools no longer
  call it: their snapshot is taken by their setup-engine job
  (`createSnapshotWithFallback`, the same discovery from the client's own
  answer) before the first write; `takeLxcSnapshot` remains for the
  project-lifecycle and project-config tools that still snapshot in the
  tool (A-17.10 / A-17.11).
