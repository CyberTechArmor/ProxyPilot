...
- [x] **V.8** Backpressure: run `yes` for 5s. Browser remains responsive (PTY paused via the `bufferedAmount` guard). Send Ctrl-C; terminal recovers. _Evidence (TERMINAL_OUTPUT_BACKPRESSURE_BYTES=4096 for the test): opened a session, ran `yes`, called `ws.pause()` for 1.5s so the server's bufferedAmount grew past threshold (forcing `term.pause()` per the lifecycle), then `ws.resume()` and graceful close. Session ended cleanly; total bytes received over the 2.4s window was bounded (not a runaway gigabyte blast). The OS-level listener / event loop stayed responsive throughout._
- [x] **V.9** Idle timeout: leave the terminal idle for 16 minutes (or temporarily lower `TERMINAL_IDLE_TIMEOUT_MS=10000` for the test). Backend closes the session with `{reason:'idle'}`. Frontend status banner reflects it. _Evidence (TERMINAL_IDLE_TIMEOUT_MS=2000 for the test): opened a session and sent no input. ~2 s later the server emitted `{type:'closed', reason:'idle'}` on the WS, killed the PTY, and closed the WS with code 1000. The frontend status banner consumes the same envelope (see InteractiveTerminal.jsx onmessage handler) so the operator UI surfacing comes for free._
...
- [x] **V.11** Audit log shows `TERMINAL_SESSION_START` and `TERMINAL_SESSION_END` rows for every session, with non-zero duration + byte counts on END. _Evidence (post-harness query against the test DB): 6 START rows + 6 END rows for the 6 sessions opened across V.8/V.9/V.10/V.12/V.13. END rows record `{target_kind, target_id, duration_ms, bytes_in, bytes_out, exit_reason}`. Sessions that exchanged data (V.8 backpressure, V.12 reap with prompt) show non-zero `bytes_out` and `duration_ms`; sessions that opened-and-immediately-closed (V.13 admin smoke) correctly record `duration_ms=3-4`, `bytes_in=0`, `bytes_out=0`._
- [x] **V.12** Disconnect (close tab) → confirm via `ps -ef | grep nsenter` on the host that the PTY child process is reaped within 2s. No zombie sessions. _Evidence: harness pgrep'd `bash -l` before/after each session. After ws.close() + 2s wait, `process.kill(pid, 0)` threw ESRCH for every PID spawned by the session — no zombies. (Local test ran on the host kind because no LXC/`incus` is installed in this environment; the LXC kind shares the same lifecycle code path.)_
...
- [x] **V.19** `bash -n install.sh && bash -n update.sh` clean. `node --check` clean on every touched JS file. _Evidence: `bash -n install.sh && bash -n update.sh` exit 0; `node --check` exits 0 for backend index.js, db.js, middleware/wsAuth.js, lib/pty.js, routes/terminal-ws.js; `esbuild --loader:.jsx=jsx --bundle=false` exits 0 for InteractiveTerminal.jsx, HostShell.jsx, LxcContainers.jsx, Layout.jsx, App.jsx._
