...
case "$1" in
    version) echo 6.0.4 ;;
    list) echo '[]' ;;
    query) printf '{"metadata":{"environment":{"server_clustered":%s}}}\n' "$MOCK_CLUSTERED" ;;
    *) exit 99 ;;
esac
EOF
...
case_refuses() {
    local cluster="$1" kernel="$2" expected="$3" output status
    rm -f "$fixture/apt-called"
    status=0
    output=$(PATH="$fixture:$PATH" MOCK_CLUSTERED="$cluster" MOCK_KERNEL="$kernel" \
...
case_refuses true 6.12.0 'cluster status is unknown or clustered'
